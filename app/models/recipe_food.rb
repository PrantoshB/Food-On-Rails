class RecipeFood < ApplicationRecord
  ...
  def value
    return "no food error" if !self.food
    self.food.price * self.quantity
  end

  def self.total_price(recipe_foods)
    total_price = recipe_foods.reduce(0) { |sum, recipe_food| sum += recipe_food.value}
  end
  ...
  def self.shopping_list(user)
    ...
    recipe_foods.each do |recipe_food|
      ...
      if food.length === 1
        ...
        shop_list << recipe_food if recipe_food.quantity > 0
      else
